/**
 * 每日自动备份脚本 - 备份到 Cloudflare R2
 * 
 * 使用方法:
 *   npx tsx scripts/daily-backup.ts
 * 
 * 定时任务设置 (Linux crontab):
 *   0 2 * * * cd /path/to/project && npx tsx scripts/daily-backup.ts >> /var/log/backup.log 2>&1
 * 
 * Windows 计划任务:
 *   在任务计划程序中创建任务，每天凌晨2点执行：
 *   程序: cmd
 *   参数: /c cd /d D:\code\cursor\topai && npx tsx scripts/daily-backup.ts
 * 
 * 功能:
 *   - 导出所有数据库数据为 JSON 格式
 *   - 上传备份文件到 Cloudflare R2 的 backupdb/ 文件夹
 *   - 自动清理超过指定天数的旧备份
 *   - 支持 gzip 压缩
 *   - 可通过 API 接口或 cron 调用
 * 
 * 备份存储位置:
 *   https://images.topai.ink/backupdb/backup_YYYY-MM-DD_HH-MM-SS.json.gz
 */
...
async function uploadBackupToR2(data: object, fileName: string): Promise<boolean> {
  ...
  try {
    ...
    console.log(`   公开访问 URL: ${R2_PUBLIC_URL}/${fileName}`);
    return true;
  } catch (error) {
...
async function main(): Promise<void> {
  ...
  console.log('🗄️  每日数据库备份 - Cloudflare R2');
  console.log('='.repeat(60));
  console.log(`时间: ${new Date().toLocaleString('zh-CN')}`);
  console.log(`保留天数: ${RETENTION_DAYS}`);
  console.log(`备份位置: ${R2_PUBLIC_URL}/${BACKUP_PREFIX}`);
  console.log('');
