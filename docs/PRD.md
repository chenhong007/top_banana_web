...
| 全宽搜索框 | 搜索框宽度与卡片网格两端对齐，提升视觉一致性 | P1 |
...
| 左侧导航栏筛选 | 桌面端将生成类型和AI模型筛选移至左侧导航栏，采用纵向布局 | P1 |
...
| effect | 文本 | ✅ | 效果/标题描述 |
| source | URL | ✅ | 来源链接 |
| category | 选择 | ✅ | 生成类型（文生图/文生视频/文生音频等） |
| description | 文本 | ✅ | 详细说明 |
| prompt | 长文本 | ✅ | 提示词内容 |
...
**表单验证规则**：
- 所有必填字段均用红色星号（*）标记
- 用户点击提交时，若必填字段未填写完整，将显示验证错误提示
- 未填写的必填字段输入框边框显示红色，并在字段下方显示具体错误信息
- 表单底部显示缺失字段汇总列表，便于用户快速定位
...
| URL 上传 | 从网络 URL 下载并存储到 R2（强制上传，不支持直接使用外部 URL） | 导入外部图片资源 |
...
│   │   ├── components/          # 前端专用组件
│   │   │   ├── home/            # 首页子组件
│   │   │   │   ├── Sidebar.tsx  # 左侧导航栏（生成类型+AI模型筛选）
│   │   │   │   ├── SearchBox.tsx # 搜索框
│   │   │   │   ├── CategoryFilter.tsx # 生成类型筛选
│   │   │   │   ├── ModelTagFilter.tsx # AI模型筛选
│   │   │   │   ├── TagFilter.tsx # 场景标签筛选
